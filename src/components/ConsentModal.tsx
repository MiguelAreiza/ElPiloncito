...
interface Props {
    ...
    type: 'cookies' | 'notifications' | 'geolocation'
    addToastr: (message: string, type?: 'success' | 'info' | 'warning' | 'error', time?: number) => void
}

function ConsentModal({ consent, setConsent, type, addToastr }: Props) {
            ...
            if (permission === 'granted') {
                ...
                if (token) {
                    onMessage(messaging, message => {
                        if (message.notification?.body) addToastr(message.notification.body);
                    });
                }
            }
        } else {
